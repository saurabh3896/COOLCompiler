/*
	Authors: Sahil Manocha, Arish Ojaswi
	This code correctly annotates the AST with types. 
*/
...
*/
	...
	private void ProcessGraph(List <AST.class_> classes) {
		...
		List <String> no_inherit = Arrays.asList("String", "Int", "Bool", "SELF_TYPE");
		...
		for(AST.class_ e : classes) {
			if(no_redef.contains(e.name)) {
				reportError(e.filename, e.lineNo+1, "Redefinition of basic class " + e.name+".");
				System.out.println("Compilation halted due to static semantic errors.");
				System.exit(1);
			}
			else if(no_inherit.contains(e.parent)) {
				reportError(e.filename, e.lineNo + (e.parent.equals("SELF_TYPE")?0:1), "Class " + e.name + " cannot inherit class " + e.parent + ".");
				System.out.println("Compilation halted due to static semantic errors.");
				System.exit(1);
			}
				...
				reportError(e.filename, e.lineNo, "Class " + e.name + " was previously defined.");
		...
		for(AST.class_ e : classes) {
			if(classIdx.containsKey(e.parent) == false) {
				reportError(e.filename, e.lineNo, "Class " + e.name + " inherits from an undefined class " + e.parent + ".");
				System.err.println("Compilation halted due to static semantic errors.");
				System.exit(1);
			}
	...
	private void ProcessNode(AST.method method) {
		...
		for(AST.formal e : method.formals) {
			...
			if(e.name.equals("self")) {
				reportError(filename, e.lineNo, "'self' cannot be the name of a formal parameter.");
			}
		...
		if(classTable.conforms(currentClass, method.body.type, method.typeid) == false) {
	...
	private void ProcessNode(AST.attr attr) {
		...
		if(attr.name.equals("self")){
			reportError(filename, attr.value.lineNo, "'self' cannot be the name of an attribute.");
		}
		else if(attr.value.getClass() != AST.no_expr.class) {
			// if return type conforms to the method type, then hurray
			if(classTable.conforms(currentClass, attr.value.type, attr.typeid) == false) {
	...
	private void ProcessNode(AST.assign assign) {
		...
		else if(classTable.conforms(currentClass, assign.e1.type, a.typeid) == false)
		...
		else if(assign.name.equals("self")) {
	...
	private void ProcessNode(AST.static_dispatch sd) {
		...
		else if(classTable.conforms(currentClass, sd.caller.type, c.name) == false)
			...
			if(c.mlist.containsKey(sd.name)) {
				...
				if(sd.actuals.size() != m.formals.size())
					...
					for(int i = 0; i < sd.actuals.size(); ++i) {
						...
						if(classTable.conforms(currentClass, actual_type, formal_type) == false)
							reportError(filename, sd.lineNo, "In call of method " + m.name + ", type " + actual_type + " of parameter "+m.formals.get(i).name+" does not conform to declared type " + formal_type + ".");			
					}
				}	
			}
			else {
	...
	private void ProcessNode(AST.dispatch dispatch) {
			...
			if(c.mlist.containsKey(dispatch.name)) {
				...
				if(dispatch.actuals.size() != m.formals.size())
					...
					for(int i = 0; i < dispatch.actuals.size(); ++i) {
						...
						String formal_type = m.formals.get(i).typeid;
						if(classTable.conforms(currentClass, actual_type, formal_type) == false)
							reportError(filename, dispatch.lineNo, "In call of method " + m.name + ", type " + actual_type + " of parameter "+m.formals.get(i).name + " does not conform to declared type " + formal_type+ ".");			
					}
				}	
			}
			else {
		...
		if(found){
			if(m.typeid.equals("SELF_TYPE")) {
				if(dispatch.caller.type.equals("SELF_TYPE")) {
					dispatch.type = currentClass;
				} else {
					dispatch.type = dispatch.caller.type;
				}
			} else {
	...
	private void ProcessNode(AST.let let) {
		...
		if(let.value.getClass() != AST.no_expr.class) {
			...
			if(classTable.conforms(currentClass, let.value.type, let.typeid) == false)
				reportError(filename, let.lineNo + 1, "Inferred type " + let.value.type + " of initialization"
						+ " of " + let.name + " does not conform to identifier's declared type " + let.typeid + ".");
		}
		scopeTable.enterScope();
	...
	private void ProcessNode(AST.new_ new_) {
		String className = new_.typeid.equals("SELF_TYPE")?currentClass:new_.typeid;
		ClassPlus c = classTable.getClassPlus(className);
		if(c == null) {
			reportError(filename, new_.lineNo, "'new' used with undefined class " + currentClass);
			new_.type = "Object";
		} else
			new_.type = className;
	}
